...
# # creating the ARIMA model automatically without calculating the values of p,d,q
# 
# plot(forecast(fit,12)) # Data forecasted for a year
# 
# # Data forecasted for a year using exponential time series
# plot(forecast(ets(AirPassengers),12))
# 
# # Data forecasted for a year using Neural Network
# plot(forecast(nnetar(AirPassengers),12))
# 
# 
# 
# # Checking the model created by different 
# 
# check<-ts(AirPassengers,start=1948,frequency = 12)
# start(check)
# check
# end(check)
# 
# fit1<-auto.arima(check)
# 
# pred1<-predict(fit1,n.ahead = 10*12)
# 
# ts.plot(head(pred1$pred,12),tail(AirPassengers,12),col=c('red','green'))
